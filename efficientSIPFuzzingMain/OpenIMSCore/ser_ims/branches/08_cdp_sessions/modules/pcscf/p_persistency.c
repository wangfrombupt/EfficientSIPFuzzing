...
int make_snapshot_dialogs()
{
	...
	if(pcscf_persistency_mode!=WITH_DATABASE_CACHE){
	...
	i = p_dump(&x,pcscf_persistency_location,"pdialogs", P_DIALOGS);
...
int load_snapshot_dialogs()
{
	...
	if (!p_load(&x,pcscf_persistency_location,"pdialogs",P_DIALOGS)) goto error;
...
int make_snapshot_registrar()
{
	...
	i = p_dump(&x,pcscf_persistency_location,"pregistrar", P_REGISTRAR);
...
int load_snapshot_registrar()
{
	...
	if (!p_load(&x,pcscf_persistency_location,"pregistrar",P_REGISTRAR)) goto error;
...
int make_snapshot_subscriptions()
{
	...
	i = p_dump(&x,pcscf_persistency_location,"psubscriptions", P_SUBSCRIPTIONS);
...
int load_snapshot_subscriptions()
{
	...
	if (!p_load(&x,pcscf_persistency_location,"psubscriptions",P_SUBSCRIPTIONS)) goto error;
 ...
 * @param x - binary data to dump. Empty if pcscf_persistency_mode==WITH_DATABASE_CACHE.
...
int p_dump(bin_data* x, char* location, char* prepend_fname, data_type_t dt){
	switch (pcscf_persistency_mode){
		case NO_PERSISTENCY:
			LOG(L_ERR,"ERR:"M_NAME":p_dump: Snapshot done but persistency was disabled...\n");
			return 0;
		case WITH_FILES:
		...
		default:
			LOG(L_ERR,"ERR:"M_NAME":p_dump: Snapshot done but no such mode %d\n",pcscf_persistency_mode);
			return 0;
	}
}
 ...
 * @param x - destination of the loaded data. Empty if pcscf_persistency_mode==WITH_DATABASE_CACHE.
...
int p_load(bin_data *x, char* location, char* prepend_fname, data_type_t dt){
	switch (pcscf_persistency_mode){
		case NO_PERSISTENCY:
			LOG(L_ERR,"ERR:"M_NAME":p_load: Persistency support was disabled\n");
			return 0;
		case WITH_FILES:
		...
		default:
			LOG(L_ERR,"ERR:"M_NAME":p_load: Can't resume because no such mode %d\n",pcscf_persistency_mode);
			return 0;
	}
}
