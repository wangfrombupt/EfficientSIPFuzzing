/**
 * $Id: diameter_avp.c 513 2008-01-04 12:13:01Z albertoberlios $
 *  
...
str AAAGroupAVPS(AAA_AVP_LIST avps)
 {
	...
	if ((char*)p-buf.s!=buf.len) {
		LOG(L_ERR,"BUG:hss3g_group_avps: mismatch between len and buf!\n");
		shm_free( buf.s );
		buf.s = 0;
		buf.len = 0;
		return buf;
...
AAA_AVP_LIST AAAUngroupAVPS(str buf)
{
	...
	while (ptr < buf.s+buf.len) {
		if (ptr+AVP_HDR_SIZE(0x80)>buf.s+buf.len){
			LOG(L_ERR,"ERROR:hss3g_ungroup_avps: source buffer to short!! "
		...
		ptr += AVP_LENGTH_SIZE;
		if (avp_len<1) {
			LOG(L_ERR,"ERROR:hss3g_ungroup_avps: invalid AVP len [%d]\n",
				avp_len);
			goto error;
		}
		/* avp vendor-ID */
		...
		if (!avp) {
			LOG(L_ERR,"ERROR:hss3g_ungroup_avps: can't create avp for member of list\n");
			goto error;
		}
