...
/*		List<IMPU> [] impu_array;
		...
		*/
		...
		List impus_list = new ArrayList();   // "initial_impus_list" re-formatted into a matrix-like list
		while (iter.hasNext()){
			IMPU impu = (IMPU) iter.next();
			List aux = new ArrayList(); // Insert every IMPU in a List format
			aux.add(impu);				//
...
*/
		...
		for (int i=0; i < (ifcs_list.size()); i++){
			...
			while (j < ifcs_list.size())
			{
				...
				if (exit==false){
					export = (List) impus_list.get(i);
					export.add(((List)impus_list.get(j)).get(0));
					impus_list.remove(j);
					ifcs_list.remove(j);
					Sps.remove(j);
